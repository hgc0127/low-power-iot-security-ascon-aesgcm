...
Fatal Weakness
AES-GCM uses block-based encryption, resulting in larger packet overhead. In the constrained 127-byte environment of IEEE 802.15.4, this overhead triggers fragmentation as data size increases, consuming more energy.
...
In the simplified implementation, AES-GCM appeared faster. However, this does not reflect the actual algorithmic structure. The 32-byte values were re-measured in Cooja using a Sky mote simulation after presentation feedback.
...
As data size increases, ASCON's energy efficiency surpasses AES-GCM. The 32-byte ASCON and AES-GCM points are based on additional Cooja implementation runs with host-side timing calibration.
...
Visualization
Energy Consumption Bar Chart Energy Consumption Line Chart Average Energy with Std Dev
...
The energy difference is not merely due to algorithmic computation. AES-GCM's block-based processing triggers fragmentation within the IEEE 802.15.4 packet constraint, leading to additional energy consumption.
...
Add UDP server/client nodes (rpl-udp example)
Add ASCON/AES-GCM nodes
Start/Pause → Check Mote output
9. References
